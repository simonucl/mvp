...
do
    ...
    do
        ...
        do 
            ...
            nohup python3 main.py --mode attack \
                                        --attack_name ${ATTACK} --num_examples 1000 --dataset ${DATASET} \
                                        --query_budget 1000 --batch_size ${BATCH_SIZE} --model_type ${MODEL_TYPE} --model ${MODEL} \
                                        --verbalizer_file ${VERBALIZER_FILE} --template_file ${TEMPLATE_FILE} \
                                        --seed $SEED --shot ${SHOT} \
                                        --adv_augment $ADV --knn_k $KNN --is_quantized --precision int8 --model_dir ${MODELPATH}_uniform_${label_dist}_quantized --label_threshold ${label_dist} --max_percent_words 0.5 > ${MODELPATH}/logs_${ATTACK}_uniform_${label_dist}_quantized.txt
        done
    done
done
