...
for ATTACK in textbugger textfooler swap_labels irrelevant_sample;
do
    if [[ $ATTACK -eq "textfooler" ]] || [[ $ATTACK -eq "textbugger" ]] || [[ $ATTACK -eq "icl_attack" ]]; then
        ATTACK_PRECENT=0.15
    else
        ATTACK_PRECENT=0.5
    fi

    if [[ $ATTACK -eq "textbugger" ]]; then
        SEEDS=(1)
        SHOTS=(2)
    else
        SEEDS=(1 13 42)
    ...
    if [[ $ATTACK -eq irrelevant_sample ]]; then
        SHOTS=(16)
    fi

    if [[ $ATTACK -eq "textfooler" ]] || [[ $ATTACK -eq "textbugger" ]]; then
        MODEL_TYPE="knn_icl"
    else
