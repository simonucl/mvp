BATCH_SIZE=$1
DATASET=$2
MODEL=$3
MODEL_TYPE=$4
TEMPLATE_FILE=${5}
VERBALIZER_FILE=${6}
ATTACK=${7}
ADV=${8}


# source ~/.bashrc
# echo $PWD
# conda activate /home/co-huan1/rds/rds-qakg-2iBGk7DbOVc/jie/conda/multi

# export XLA_FLAGS=--xla_gpu_cuda_data_dir=/usr/local/software/spack/spack-rhel8-20210927/opt/spack/linux-centos8-zen2/gcc-9.4.0/cuda-11.4.0-3hnxhjt2jt4ruy75w2q4mnvkw7dty72l

for ATTACK in swap_labels textfooler;
do
    if [[ $ATTACK == "textfooler" ]]; then
        MODEL_TYPE=icl
    else
        MODEL_TYPE=icl_attack
    fi

    for SHOT in 8 4;
    do
        for SEED in 1 13 42;
        do 
            # for precision in int4 int8 float16 bfloat16;
            for precision in int4 float16;
            do
                echo $SEED+${SHOT}+${MODEL}+"mvp"

                MODEL_ID=${MODEL_TYPE}-seed-${SEED}-shot-${SHOT}
                
                # ATTACK=textfooler
                MODELPATH=./checkpoints/${DATASET}/${MODEL}/${ATTACK}/${MODEL_ID}

                DATASET_PATH=./data/${DATASET}/${SHOT}-$SEED

                mkdir -p ${MODELPATH}
                echo ${MODELPATH}

                # MODEL_TYPE=knn_icl
                KNN=4
                if [[ ${precision} == "int4" ]] || [[ ${precision} == "int8" ]]; then
                    BATCH_SIZE=$((64 / SHOT))
                else
                    BATCH_SIZE=$((8 / SHOT))
                fi
                # nohup python3 main.py --mode attack \
                #                             --attack_name ${ATTACK} --num_examples 1000 --dataset ${DATASET} \
                #                             --query_budget -1 --batch_size ${BATCH_SIZE} --model_type ${MODEL_TYPE} --model ${MODEL} \
                #                             --verbalizer_file ${VERBALIZER_FILE} --template_file ${TEMPLATE_FILE} \
                #                             --seed $SEED --shot ${SHOT} \
                #                             --adv_augment $ADV --knn_k $KNN --max_percent_words 0.15 > ${MODELPATH}/logs_${ATTACK}.txt

                if [[ ${precision} == "int4" ]] || [[ ${precision} == "int8" ]]; then
                nohup python3 main.py --mode attack \
                                            --attack_name ${ATTACK} --num_examples 1000 --dataset ${DATASET} \
                                            --query_budget -1 --batch_size ${BATCH_SIZE} --model_type ${MODEL_TYPE} --model ${MODEL} \
                                            --verbalizer_file ${VERBALIZER_FILE} --template_file ${TEMPLATE_FILE} \
                                            --seed $SEED --shot ${SHOT} \
                                            --adv_augment $ADV --knn_k $KNN --is_quantized --precision ${precision} --model_dir ${MODELPATH}_quantized > ${MODELPATH}/logs_${ATTACK}_quantized_${precision}.txt
                else
                nohup python3 main.py --mode attack \
                                            --attack_name ${ATTACK} --num_examples 1000 --dataset ${DATASET} \
                                            --query_budget -1 --batch_size ${BATCH_SIZE} --model_type ${MODEL_TYPE} --model ${MODEL} \
                                            --verbalizer_file ${VERBALIZER_FILE} --template_file ${TEMPLATE_FILE} \
                                            --seed $SEED --shot ${SHOT} \
                                            --adv_augment $ADV --knn_k $KNN --precision ${precision} --model_dir ${MODELPATH}_quantized > ${MODELPATH}/logs_${ATTACK}_quantized_${precision}.txt
                fi

            done
        done
    done
done
